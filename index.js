...
// Phục vụ file index.html ở trang chủ
app.get('/', (req, res) => {
...
app.post('/api/translate', async (req, res) => {
  ...
  // Prompt gọn gàng hơn, xóa thụt lề, trim để giảm token
  const prompt = `
...
- "Ngôi Nhà Nhỏ" (小房子) là tổ hợp kinh văn đặc biệt để trả nợ nghiệp, siêu độ vong linh, người cần kinh (要经者) và thai nhi bị phá bỏ.
- Các kinh văn thường dùng: Chú Đại Bi (大悲咒), Tâm Kinh (心经), Lễ Phật Đại Sám Hối Văn (礼佛大忏悔文), Chú Chuẩn Đề (准提神咒), Giải Kết Chú (解结咒).
- Giấc mơ (梦境) thường báo hiệu nghiệp chướng, vong linh cần siêu độ, hoặc điềm báo.
- Các vấn đề của trẻ nhỏ thường liên quan đến nghiệp chướng của cha mẹ.
**Văn bản cần dịch:**
---
${chineseText}
---
`.trim();
  ...
  try {
    ...
    if (!geminiResponse.ok) {
      ...
      console.error('❗ Lỗi từ Google AI API:', errorResult);
      throw new Error(errorResult.error?.message || 'Lỗi từ Google AI API');
    }

    res.setHeader('Content-Type', 'text/plain; charset=utf-8');

    // Xử lý lỗi trong khi stream
    geminiResponse.body.on('error', (streamErr) => {
      console.error('❗ Streaming error:', streamErr);
      res.end(); // Ngắt kết nối tới client
    });
